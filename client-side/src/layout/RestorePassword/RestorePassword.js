...
const RestorePassword = () => {
  ...
  const submitRestorePassword = (ev) => {
    ev.preventDefault();
    const passInput = ev.target.newPassword[0];
    ...
    if (
      ...
      pass.length > 0 &&
      ev.target.confirmNewPassword[0].value.trim() === pass
    ) {
  ...
  const handleBlurInput = (ev) => {
    ...
    if (target === "newPassword") {
      ...
      if (val.length > 0) {
        const newPasswordValue = ev.target.parentElement.parentElement.parentElement.parentElement.firstChild.firstChild.firstChild.firstChild.value.trim();
  ...
  const toggleShowPassword = (ev) => {
    ev.target.parentElement.firstChild.firstChild.firstChild.type =
      ev.target.parentElement.firstChild.firstChild.type === "text"
        ? "password"
        : "text";
  };
